...
import { Event,  RSVP } from "@/lib/models/event.model";
...
export const POST = async(req:Request)=>{
    ...
    try {
        ...
        const rsvps = await RSVP.updateMany({_id:{$in:selections}},{isAccepted:false,status:'rejected'});
        return Response.json({success:true,message:"RSVPS accepted successfully"});
    } catch (error) {
        console.log(error)
