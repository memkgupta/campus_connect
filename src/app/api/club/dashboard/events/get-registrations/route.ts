  ...
  try {
    ...
    const registrations = await EventRegistration.aggregate([
        ...
        {$limit:10}
            ...
            const total = await EventRegistration.find({ event:queryParams.event_id,
                isAccepted:false}).countDocuments();
        return Response.json({success:true,registrations:registrations,total:total},{status:200});
